...
} from './FibonacciDrawings/FibonacciDrawingLevelLineExtension';

import {
    DrawingTextHorizontalPosition,
    DrawingTextVerticalPosition
} from './DrawingTextPosition';
...
export class DrawingsDefaultSettings {

    //private variables
    private static savedDefaultSettings: { [drawingClassName: string]: DrawingDefaultSettings } = null;

    //public methods
    public static getDrawingDefaultSettings(themeType:ThemeType, className: string): DrawingDefaultSettings {
        let settings: DrawingDefaultSettings = DrawingsDefaultSettings.drawingSavedDefaultSettings[className] || DrawingsDefaultSettings.getDrawingOriginalSettings(themeType, className);
        //NK return a copy of the saved or original drawing settings to give the drawing the right to change it with it's own fields
        return $.extend(true, {}, settings);
    }

    public static hasCustomSettings(themeType:ThemeType, className: string): boolean {
        return DrawingsDefaultSettings.drawingSavedDefaultSettings[className] != null;
    }

    public static setDrawingDefaultSettings(themeType:ThemeType, className: string, settings: IDrawingOptions) {
        let originalOptions: DrawingDefaultSettings = DrawingsDefaultSettings.getDrawingOriginalSettings(themeType, className);
        let optionsWeShouldSave: DrawingDefaultSettings = {} as DrawingDefaultSettings;
    ...
    public static getResettedDrawingSettings(themeType:ThemeType, className: string, currentOptions: IDrawingOptions): IDrawingOptions {
        DrawingsDefaultSettings.drawingSavedDefaultSettings[className] = null;
        DrawingsDefaultSettings.writeDrawingDefaultSettings();

        let originalOptions: DrawingDefaultSettings = DrawingsDefaultSettings.getDrawingOriginalSettings(themeType, className);

        let mergedOptions: IDrawingOptions = {} as IDrawingOptions;
        for (let option of Object.keys(currentOptions)) {
            mergedOptions[option] = originalOptions[option] ? originalOptions[option] : currentOptions[option];
        }
        return mergedOptions;
    }

    public static resetAllSavedSettings() {
        DrawingsDefaultSettings.savedDefaultSettings = null;
        DrawingsDefaultSettings.writeDrawingDefaultSettings();
    }

    //private methods
    private static get drawingSavedDefaultSettings(): { [drawingClassName: string]: DrawingDefaultSettings } {
    ...
    public static getDrawingOriginalSettings(themeType: ThemeType, className: string): DrawingDefaultSettings {
    ...
    private static getDrawingOriginalDarkSettings(className:string): DrawingDefaultSettings{
        switch (className) {
            case 'lineSegment':
            case 'trendChannel':
    ...
    private static getDrawingOriginalLightSettings(className:string): DrawingDefaultSettings {
        switch (className) {
            case 'lineSegment':
            case 'trendChannel':
            case 'andrewsPitchfork':
