import {ChartTheme, IStrokeTheme, KumoPlotTheme, LinePlotTheme, LineStyle} from '../Theme';
import {DataSeriesSuffix} from "../Data/DataSeries";
import {
    AverageTrueRange,
    Const,
    ExponentialMovingAverage, DoubleExponentialMovingAverage, PriceOscillator, RainbowOscillator, SimpleMovingAverage, TimeSeriesMovingAverage,
    TriangularMovingAverage,
    TRIX, TrueRange,
    UltimateOscillator,
    VariableMovingAverage, VerticalHorizontalFilter, VIDYA, VolumeOscillator, WeightedMovingAverage,
    ...
    LiquidityByValue,
    LiquidityByVolume,
    VolumeIndicator, VolumeProfilerSessionVolume, VolumeProfilerVisibleRange,
...
export class IndicatorsDefaultSettings {

    //private variables
    private static savedDefaultSettings: { [indicatorNumber: number]: IndicatorSettings } = null;

    //public methods
    public static getIndicatorDefaultSettings(themeType:ThemeType, indicatorNumber: number): IndicatorSettings {
        let settings = IndicatorsDefaultSettings.indicatorsSavedDefaultSettings[indicatorNumber] ||
            IndicatorsDefaultSettings.getIndicatorOriginalSettings(themeType, indicatorNumber);
        return $.extend(true, {}, settings);
    }

    public static setIndicatorDefaultSettings(indicatorNumber: number, parameters: {[key: string]: ParameterValueType}, horizontalLines: HorizontalLine[]) {
    ...
    public static resetIndicatorSettings(themeType:ThemeType, indicatorNumber: number): IndicatorSettings {
    ...
    //private methods
    private static get indicatorsSavedDefaultSettings(): { [indicatorNumber: number]: IndicatorSettings } {
    ...
    private static getIndicatorOriginalSettings(themeType:ThemeType, indicatorNumber: number): IndicatorSettings {
        return {
    ...
    private static getIndicatorParam(themeType:ThemeType, indicatorNumber: number): {[key: string]: ParameterValueType}  {
        let params: {[key: string]: ParameterValueType} = {};

        let theme = themeType == ThemeType.Light ? IndicatorTheme.Light : IndicatorTheme.Dark;
    ...
    private static getIndicatorHorizontalLine(themeType:ThemeType, indicatorNumber: number): HorizontalLine[] {
