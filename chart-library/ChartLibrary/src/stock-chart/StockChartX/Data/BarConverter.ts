...
export class BarConverter {
    private static _setupOhlcDataSeries(suffix: string, dataSeries?: IOHLCDataSeries): IOHLCDataSeries {
    ...
    private static _setupBarDataSeries(suffix: string, dataSeries?: IBarDataSeries): IBarDataSeries {
        let dsSuffix = DataSeriesSuffix;
    ...
    //            let atr = new Indicator({
    //                taIndicator: AverageTrueRange,
    //                chart: this
    //            });
    //            atr.setParameterValue(.IndicatorParam.PERIODS, 20);
            ...
            if (isRaising) {
                ...
                if (price < highBound - reversal) {
                    let newHighBound = highBound - boxSize,
                        newLowBound = lowBound - Math.round((lowBound - price) / boxSize) * boxSize;
                ...
                if (price > lowBound + reversal) {
                    let newLowBound = lowBound + boxSize,
                        newHighBound = highBound + Math.round((price - highBound) / boxSize) * boxSize;
