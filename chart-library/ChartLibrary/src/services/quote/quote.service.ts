...
import {Company, CompanyFlag, Market, Sector} from '../loader';
...
import {MessageBoxRequest} from "@src/services/shared-channel/channel-request";
...
export class QuoteService {
    ...
    private createQuotes(market:Market) {
        market.sortedCompanies.forEach(company => this.createQuote(market, company));
    }
    ...
    private createQuote(market:Market, company:Company) {
	  ...
	  let quote = new Quote(company, market.abbreviation, companyFlag, sector, true);
        // let quote = new Quote(company, market.abbreviation, companyFlag, sector, this.authorizationService.isSubscriber());
    ...
    private startSnapshotTimer() {
        ...
        // }, QuoteService.SNAPSHOT_WAIT_TIME_PER_MARKET * this.marketsManager.getAllSubscribedMarkets().length);
		}, QuoteService.SNAPSHOT_WAIT_TIME_PER_MARKET * 1);
    }
