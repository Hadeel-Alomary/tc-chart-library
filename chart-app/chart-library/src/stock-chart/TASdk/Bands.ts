...
export class Bands {
    ...
    keltner(pOHLCV: Recordset, Periods: number, Factor: number, MAType: number, Alias: string): Recordset {
        ...
        let median = ma.movingAverageSwitch(pOHLCV.getField('Close'), Periods, MAType, Alias + ' Median').getField(Alias + ' Median');
    ...
    ichimoku(pOHLCV: Recordset, ConversionLinePeriods: number, BaseLinePeriods: number, LoggingSpan2Periods: number): Recordset {
        ...
        for (let record = recordCount + 1; record >= 0; record--) {
            ...
            let currentParameters: {[key: string]: number} = {
